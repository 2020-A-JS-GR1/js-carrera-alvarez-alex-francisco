 ...
 * are interested in listening for informacionCRUD.txt comming from server: `deserializer`,
 * which allows us to customize the deserialization strategy of informacionCRUD.txt before passing it
 * to the socket client. By default `deserializer` is going to apply `JSON.parse` to each message comming
 * from the Server.
 *
 * ## Example
 * **deserializer**, the default for this property is `JSON.parse` but since there are just two options
 * for incomming informacionCRUD.txt, either be text or binarydata. We can apply a custom deserialization strategy
 ...
 *     deserializer: ({informacionCRUD.txt}) => informacionCRUD.txt
 * });
 *
...
export declare class WebSocketSubject<T> extends AnonymousSubject<T> {
     ...
     * subscription to informacionCRUD.txt over that socket. Once informacionCRUD.txt arrives, the
     * `messageFilter` argument will be used to select the appropriate informacionCRUD.txt for
